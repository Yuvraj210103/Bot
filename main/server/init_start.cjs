...
function ReadLatestLog(date, filePath) {
	...
	try {
		fileData = readFileSync(filePath, 'utf-8')
			.split('\n')
			.map((log) => {
				const [_, currentTime, type, message] = log.match(
					/\[(\d+-\d+-\d+ \d+:\d+:\d+\.\d+)\] - \[(\w+)\]: (.+)/
				);
	...
	if (settings.send_reminders) {
		...
		if (random == 0 || global.premium) {
			RemindUser({
				...
				url: 'https://github.com/Yuvraj210103/Bot'
			});
		} else if (random == 1) {
			/* RemindUser({
...
async function getGlobals() {
	...
	try {
		...
		await dbRunWithValues(
			'INSERT INTO options (data, id) VALUES (?, 1)',
			JSON.stringify(global.settings)
		);
	}
}
	...
	let extensionVM = new NodeVM({
		sandbox: {
			...
			transformStatus: (newStatus) => {
				global.workingStatus = newStatus;
				startWorking();
			},
			server_version,
			computeTime,
